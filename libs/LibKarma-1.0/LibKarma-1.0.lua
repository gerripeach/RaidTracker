if KarmaStub:isLib("LibKarma", 1.1, "$Revision: 61 $") then return end
...
-- i hate having to add all this code, but too many addons now, too many conflicts
...
function LibKarma:OnEvent( frame, event, ... )
	if event == "ADDON_LOADED" then
		if arg1 == self._instance.name then
			self:_HookDB()
...
function LibKarma:_OnUpdate( frame, elapsed, ... )
	...
	if u.time < u.next then	return else u.next = u.time + u.interval; u.count = u.count + 1; end
