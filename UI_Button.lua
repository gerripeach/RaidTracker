...
function RT_Button:DetailFrameItems_RarityDropdown_Update( )
	...
	RT_RarityDropdownFrameText:SetText( RT_Options.desc.rarity[ id ] )
end

function RT_DetailFrameItems_RarityDropdown_Initialize( frame, level )
	for i = 1, #RT_Options.desc.rarity do
		RT_Button:AddMenuItem( nil, nil, "rarity", nil, RT_Options.desc.rarity[i] )
	end
end
...
function RT_Button:AddMenuZone( level, what, label, value, zone )
	...
	elseif level == 2 then
		for k, v in ipairs(RaidTracker_GameReleases) do
			checked = nil
			for k1, v1 in pairs(def.Zones) do
				if v1.rel == v and k1 == zone then checked = true; end
			end
			RT_Button:AddMenuItem( level, false, nil, nil, L[RaidTracker_Tags["REL"..v]], nil, v, checked, true )
		end
...
function RT_Button:AddMenuBoss( level, what, label, value, boss )
	...
	elseif level == 2 then	
		for i,v in pairs(RaidTracker_GameReleases) do
			checked = nil
			if boss then
				for k1,v1 in pairs(def.Zones) do
					if v1.rel == v and RaidTracker_ZoneBosses[k1] then 
						for k2,v2 in pairs(RaidTracker_ZoneBosses[k1]) do
	...
	elseif level == 3 then
		...
		for k, v in pairs(zones) do
			checked = nil
			if boss and RaidTracker_ZoneBosses[v.name] then 
				for k1, v1 in pairs(RaidTracker_ZoneBosses[v.name]) do
	...
	elseif level == 4 then
		for k, v in pairs(RaidTracker_ZoneBosses[value.value]) do
			if type(v) ~= "table" then
